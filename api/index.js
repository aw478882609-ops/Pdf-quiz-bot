...
function extractQuestions(text) {
    // الخطوة 1: توحيد وتنظيف النص (لا تغيير هنا)
    text = text.replace(/\r\n/g, '\n').replace(/\r/g, '\n').replace(/\f/g, '\n').replace(/\u2028|\u2029/g, '\n');
    text = text.replace(/\n{2,}/g, '\n');
    ...
    // الأنماط والدوال المساعدة (لا تغيير هنا)
    ...
    function validateOptionsSequence(optionLines) {
        ...
        for (let j = 0; j < optionLines.length; j++) {
            const line = optionLines[j]; let currentStyle = null; let currentValue = null; let identifier = '';
    ...
    // [تعديل] منطق جديد للبحث عن كتلة السؤال بشكل أكثر مرونة
    while (i < lines.length) {
        ...
        // 1️⃣ ابحث للأمام عن بداية مجموعة خيارات صالحة
        ...
        for (let j = i + 1; j < lines.length; j++) {
            if (findMatch(lines[j], optionPatterns)) {
                ...
                // إذا لم يكن التسلسل صالحًا، فستستمر الحلقة للبحث من جديد بعد هذه الكتلة غير الصالحة
                j = k - 1; 
            }
        }
        ...
        // 4️⃣ البحث عن الإجابة (باستخدام نفس المنطق القوي السابق)
        if (k < lines.length && findMatch(lines[k], answerPatterns)) {
            ...
            i = k + 1; // تقدم إلى ما بعد الإجابة
        } else {
            i = k; // تقدم إلى ما بعد الخيارات
        }

        // 5️⃣ أضف السؤال إذا كان مكتملاً
